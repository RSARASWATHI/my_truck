<?php
	...
	function due_message_date($_get){
		$con=db_connect();
		$due_date=$_get['loan_due_date'];
		$delay_date=30;
		$noticedate = date('Y-m-d', strtotime( "$due_date + $delay_date days" ));
		insert('duemessage',array('truckno','duedate','dueamount','phonenumber'),array($_get['vehicle_no'],$noticedate,$_get['loan_due_amount'],$_SESSION['phoneno']),$con);
		header('Location:../view/add_vehicle.php');
